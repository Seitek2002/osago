      ...
      <div className="grid md:grid-cols-3 gap-6 mb-[60px]">
        {cardData.map((item, index) => (
          ...
          >
            ...
            <p className="text-[#000] text-[15px] leading-[22px]">{item.text}</p>
          </div>
        ))}
      </div>
...
const StepCard: React.FC<StepCardProps> = ({
  ...
  return (
    <div className={`bg-[${bgColor}] rounded-[20px] p-[30px] border border-[${borderColor}]`}>
        ...
        {steps.map((step, i) => (
          <div key={i}>
            <div className="text-[#000] text-[16px] list-none flex items-center gap-2">
              <span className={`border border-[${color}] py-[2px] px-[9px] rounded-full`}>
                {i + 1}
              </span>
              <li>{step}</li>
            </div>
            {i !== steps.length - 1 && (
              <div
                className={`m-0 border border-[${color}] border-dotted w-0 h-5 absolute hidden lg:block`}
                style={{ top: `${30 + i * 50}px`, left: "12px" }}
              ></div>
            )}
          </div>
        ))}
